...
// marketCategories.rules.forEach(x =>
//     markets[x.key] ? (matchedMarkets[x.key] = markets[x.key]) : null
// );

//using Array.reduce()
const matchedMarkets = marketCategories.rules.reduce((acc, curr) => {
    //filter out non existent markets
    if (markets[curr.key]) {
        acc[curr.key] = markets[curr.key];
    }
...
function sortMarkets(markets) {
    const result = {};
    Object.keys(markets)
        .sort((a, b) => markets[a].info.order - markets[b].info.order)
        .forEach(k => (result[k] = markets[k]));

    return result;
}


console.log(matchedMarkets, sortMarkets(matchedMarkets));